...
const ItemDamage_Add: React.FC = () => {
  ...
  const handleReceiptLookup = async () => {
    ...
    try {
      ...
      // Filter damaged items that match this receipt number
      const returnedFromThisReceipt = allDamagedItems.filter(
        (item: any) => item.invoice_no === response.data.receipt_no
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const successMsg =
        condition === "Good Item"
          ? "Good item return recorded successfully!"
          : "Damaged item record added successfully!";
      showNotification(successMsg, "success");
    } catch (error: any) {
      console.error("Error adding item return:", error);
